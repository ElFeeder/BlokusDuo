/*
	...
	- Modifica̵̤es p/ competi̤̣o REC2018 por Rui P. Duarte
...
void init_serial(){
	...
	for (p=0; p < ports; p++){
		if(!use_tcp){ // init serial
			...
			if (tcsetattr(serial_fd[p], TCSANOW, &tio[p]) == -1) printf("tcsetattr FAILED\n");
			if (tcflush(serial_fd[p], TCIOFLUSH)== -1) printf("tcflush FAILED\n");  // flush 
		} 
		else { // init tcp
			init_tcp(p);
		}
	...
	// descobrir os nomes das equipas
	...
	for (p=0; p<ports; p++){
		...
		usleep(10000);
		//printf("p: %d, fd: %d\n", p, serial_fd[p]);
		while (read_all(serial_fd[p], 3, team_id) != 3){
